...
Simulated Twitch viewers react to what the agent is doing. Messages are generated by the LLM in batches and appear every 3-10 seconds (1-3 seconds in Master Control Room to keep up with multi-project event volume). Random viewer tips appear ~15% of the time with token donation animations. When LLM is off, hardcoded chat messages still appear to keep the stream lively.
