...
export function seekReplay(position) {
    ...
    // Partial render: only render last 200 events up to target position
    const log = getAgentLog();
    if (log) log.innerHTML = '';
    state.inventory = {};

    const target = Math.min(position, replayEvents.length);
    const renderStart = Math.max(0, target - 200);
    for (let i = renderStart; i < target; i++) {
...
export function initReplay() {
    ...
    // Clickable progress bar to seek (debounced to prevent rapid re-renders)
